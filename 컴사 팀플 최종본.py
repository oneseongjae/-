...
def energy(): 
    print('일과 에너지에서 구하고 싶은 것을 골라주세요(힘, 이동거리, 운동에너지, 퍼텐셜에너지, 역학적에너지): ')
...
#퍼텐셜에너지를 선택했을 경우
        ...
        print('이 물체의 질량(kg), 현재 높이(m)를 입력해주세요(중력가속도= 9.8m/s*s로 계산): ')
        mass=float(input('질량(kg): '))
...
#역학적 에너지를 선택했을 경우
        ...
        print('이 물체의 질량(kg), 현재 높이(m), 속력(m/s)을 입력해주세요(중력가속도= 9.8m/s*s로 계산): ')
        mass=float(input('질량(kg): '))
        height=float(input('현재 높이(m): '))
        velocity=float(input('속력(m/s): '))
...
#이동하는 물체가 파원일 경우
    if thing == "파원":
        frequency_sound = float(input("파원의 진동수(Hz)를 입력해주세요: "))
        movingvelocity_sound = float(input("파원의 이동속도(m/s)를 입력해주세요: "))
        velocity_sound = float(input("파동의 속도(m/s)를 입력해주세요: "))
...
#이동하는 물체가 관찰자일 경우
        ...
        frequency_sound = float(input("파원의 진동수(Hz)를 입력해주세요: "))
        movingvelocity_observer = float(input("관찰자의 이동속도(m/s)를 입력해주세요: "))
        velocity_sound = float(input("파동의 속도(m/s)를 입력해주세요: "))
...
#각도를 입력했을 경우
    if factor == "각도":
        ...
        #입사각을 구할 경우
        if factor_angle == "입사각":
            angle_out_degree = float(input("굴절각(도)을 입력해주세요: "))
            ...
            if -1 <= sin_value <= 1:
                ...
                print("물리적으로 불가능한 계산값이 나왔어요. 입력값을 확인해주세요.")
        #굴절각을 구할 경우
        elif factor_angle == "굴절각":
            angle_in_degree = float(input("입사각(도)을 입력해주세요: "))
...
def projectile():
    ...
    if factor == "최대높이" :
        ...
        print("초기경사각(도), 초기속도(m/s)를 입력해주세요(중력가속도= 9.8m/s*s로 계산): ")
        angle = float(input("초기경사각(도): "))
        velocity = float(input("초기속도(m/s): "))
...
def calculate_acceleration():
    print("가속도의 법칙으로 구할 요소를 입력해주세요(가속도, 알짜힘): ")
    exfactor = ["가속도", "알짜힘"]
...
#올바른 요소를 입력했는지 확인
    ...
    if factor == "가속도":
        force = float(input("물체에 작용하는 힘(N)을 입력해주세요: "))
        mass = float(input("물체의 질량(kg)을 입력해주세요: "))

        acceleration = calculate_acceleration(force, mass)
        print("물체의 가속도는 %.2f m/s^2 입니다." %(acceleration))

    elif factor == "알짜힘":
        mass = float(input("질량(kg)을 입력해주세요: "))
        acceleration = float(input("가속도(m/s*s)를 입력해주세요: "))
        net_force = mass * acceleration
        print(f"알짜힘은 {net_force} N입니다.")

#뉴턴 3법칙
def newtonlaw_3():
    force_on_door = float(input("손에 작용하는 힘(N)을 입력해주세요: "))
